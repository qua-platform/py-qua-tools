...
qop_ip = "172.16.33.101"  # Akiva OPX1000
...
#####################################
run = "live_plotting_line_by_line"
...
if run == "live_plotting":
    ...
    opx_instrument.live_plotting(["I", "Q","iteration"], number_of_runs=100)
...
def OPX_2d_scan(simulate=False):
    with program() as prog:
        ...
        with stream_processing():
            n_st.save("iteration")
            I_st.buffer(len(gate_2_biases)).save_all("I")
            Q_st.buffer(len(gate_2_biases)).save_all("Q")
    return prog


if run == "live_plotting_line_by_line":
    # Axis1 is the most inner loop
    opx_instrument.set_sweep_parameters("axis1", gate_2_biases, "V", "Gate 2 biases")
    # Axis2 is the second loop
    opx_instrument.set_sweep_parameters("axis2", gate_1_biases, "V", "Gate 1 biases")
