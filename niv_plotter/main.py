...
class GUI(QWidget):
    ...
    def initialise_ui(self):
        ...
        self.list_widget = pg.LayoutWidget()

        self.main_layout.addWidget(self.list_widget, 0, 0)
        self.main_layout.addWidget(self.plot_grid, 0, 1)

        self.setLayout(self.main_layout)


        self.setWindowTitle("Niv's special GUI")

    def setup_list(self):

        self.list = QTableWidget()
        self.list.setRowCount(10)
        self.list.setColumnCount(2)

        # make table read-only
        self.list.setEditTriggers(QTableWidget.NoEditTriggers)
        self.list.setMinimumWidth(self.list.sizeHint().width())
        self.list.setShowGrid(False)
        self.list.setMaximumWidth(200)

        self.list_widget.addWidget(self.list)



    def setup_plots(self):

        x, y = self.qubit_array_shape
        self.plots = []

        for i in range(x):
            row = []
            for j in range(y):
                plot = self.plot_grid.addPlot()
                row.append(plot)
            self.plots.append(row)
            self.plot_grid.nextRow()

        self.add_fake_data_to_plots()

    def add_fake_data_to_plots(self):
