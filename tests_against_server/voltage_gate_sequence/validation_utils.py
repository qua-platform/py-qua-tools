...
def validate_program(samples, requested_wf_p, requested_wf_m):
    ...
    plt.legend()
    print(f"Difference between requested and simulated (+): {np.mean((wf_p[:len(requested_wf_p)] - requested_wf_p) / requested_wf_p) * 100:.2e} %")
    print(f"Difference between requested and simulated (-): {np.mean((wf_m[:len(requested_wf_m)] - requested_wf_m) / requested_wf_m) * 100:.2e} %")
    print(f"Relative sum after compensation (+): {np.sum(wf_p[:t1+1]) / np.sum(wf_p[:len(requested_wf_p)]) * 100:.2f} %")
    print(f"Relative sum after compensation (-): {np.sum(wf_m[:t1+1]) / np.sum(wf_p[:len(requested_wf_m)]) * 100:.2f} %")
    ...
    # Success criteria
    assert (np.mean((wf_p[:len(requested_wf_p)] - requested_wf_p) / requested_wf_p) < 0.1) & (np.mean((wf_m[:len(requested_wf_m)] - requested_wf_m) / requested_wf_m) < 0.1), "Simulated wf doesn't match requested wf."
    assert (np.sum(wf_p[:t1+1]) / np.sum(wf_p[:len(requested_wf_p)]) * 100 < 1) & (np.sum(wf_m[:t1+1]) / np.sum(wf_p[:len(requested_wf_m)]) * 100 < 1), "The compensation pulse leads to more than 1% error."
    assert (max(np.abs(np.diff(wf_p[:t1+1]))) < 0.5) & (max(np.abs(np.diff(wf_m[:t1+1]))) < 0.5), "The maximum voltage gradient is above 0.5 V."
