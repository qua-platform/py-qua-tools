...
# %% 2 consecutive compensation pulses - ramp rate in QUA
level_init = [0.8, -0.1]
duration_init = 3000
level_manip = [0.5, -0.3]
ramp_to_manip = 1000
...
with program() as prog:
    a = declare(fixed, value=level_init[0])
    b = declare(fixed, value=level_init[1])
    t = declare(int, value=ramp_to_manip)
    with infinite_loop_():
        play("trigger", "qdac_trigger1")
        seq.add_step(voltage_point_name="initialization", level=[a, b])
        seq.add_step(voltage_point_name="idle", ramp_duration=t)
...
# %% Simulate the QUA program and validate it with the simulator
qmm, samples = simulate_program(prog, simulation_duration=100000)
validate_program(samples, requested_wf_p, requested_wf_m)

# print(generate_qua_script(prog, config))


# %% 2 consecutive compensation pulses ramp rate and duration in QUA
level_init = [0.8, -0.1]
duration_init = 3000
level_manip = [0.5, -0.3]
ramp_to_manip = 1000
duration_manip = 10000
level_readout = [0.2, -0.2]
ramp_to_readout = 4000
duration_readout = 2000
max_compensation_amplitude = 0.45
...
with program() as prog:
    a = declare(fixed, value=level_init[0])
    b = declare(fixed, value=level_init[1])
    t = declare(int, value=duration_manip)
    tR = declare(int, value=ramp_to_manip)
    with infinite_loop_():
        play("trigger", "qdac_trigger1")
        seq.add_step(voltage_point_name="initialization", level=[a, b])
        seq.add_step(voltage_point_name="idle", ramp_duration=tR, duration=t)
        seq.add_step(voltage_point_name="readout", ramp_duration=ramp_to_readout)
        seq.add_compensation_pulse(max_amplitude=max_compensation_amplitude)
        seq.ramp_to_zero()
