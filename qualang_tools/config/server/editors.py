    ...
    if (success == "OK"):
        with open(os.path.join(UPLOAD_DIRECTORY, "config_edits.py"), "a") as myfile:
            if isinstance(value,str):
                ...
                myfile.write(f"config_editor('{pathname}', updated_value={value}, configuration=configuration)\n")
        return [
            dbc.Alert(f"Updated {pathname} = {value}", color="success")
        ]
    else:
        return [
            dbc.Alert(f"{success}", color="danger")
        ]
...
def update_click(click_number, value0, value1, pathname):
    ...
    if (success == "OK"):
        with open(os.path.join(UPLOAD_DIRECTORY, "config_edits.py"), "a") as myfile:
            myfile.write(f"config_editor('{pathname}', updated_value=('{value0}',{value1}), configuration=configuration)\n")
...
@app.callback(
    [
        Output("wf-view", "figure"),
        Output("wf-link", "href")
    ],
    [Input("wf-dropdown","value")],
    [
        State("url", "pathname"),
        State("wf-initial", "children")
    ],
    prevent_initial_call=True,
)
...
def add_optional_input(button_click, fieldcount, class_name,
    current_added_items, current_options, class_input):
    ...
    try:
        ...
        return current_added_items, current_options,\
...
@app.callback(
    [Output("component-input-add", "value"),
    Output("component-input-add", "options")
    ],
