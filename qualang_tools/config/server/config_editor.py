...
def config_editor(pathname, updated_value=None, configuration = None):
    ...
    for l in location:
        if (l == "waveforms") and (
            len(location)>1 and location[0] != "waveforms"):
            waveform_view = True
        if l == "operations":
            pulse_view = True
        if (l!= ""):
            ...
            if (isinstance(a[l], dict) or 
                (isinstance(a[l],list) and len(a[l])>0
                 and isinstance(a[l][0], dict))):
                a = a[l]
                if l in documentation:
                    documentation = documentation[l]
                else:
                    if ("properties" in documentation and 
                        l in documentation["properties"]):
                        ...
                        if ("$ref" in documentation["additionalProperties"]):
                            ...
                            for option in documentation["additionalProperties"]["oneOf"]:
                                ...
                                try:
                                    # print(option)
                                    # print(a["type"])
                                    if option["properties"]["type"]["description"].find(
                                        a["type"]) != -1:
    ...
    if "description" in documentation:
        if ("title" in documentation):
            docs.append(html.H4("%s: %s" % 
                (breadcrumb_items[-1]["label"], documentation["title"]),
                className="mb-1"))    
        docs.append(html.Div(
    ...
    if (selected != ""):
        if (isinstance(input_dict[selected], list) or
            isinstance(input_dict[selected], np.ndarray)):
            ...
            if (controlArguments.find("view=list")!= -1):
                ...
                details.append(
                    dcc.Link("view as plot",
                        href=url+f"/{selected}/[view=plot]"
                    )
                )   
                # show list
                details.append(
                    dbc.InputGroup([
                        dbc.Textarea(
                            id="input-field",
                            className="mb-3", placeholder="array",
                ...
                details.append(
                    dcc.Link("view as list",
                        href=url+f"/{selected}/[view=list]"
                    )
                )
                # show plot
                ...
                details.append(dcc.Graph(id='graph',
            ...
            if (isinstance(input_dict[selected], float)):
                ...
                details.append(
                    dbc.InputGroup(
                        [
                            ...
                            dbc.Input(id="input-field", placeholder="Enter value...",
                        type="number", value=("%.8e" % input_dict[selected])),
                        dbc.Button("Update", id="update-button", n_clicks=0),
                        ]
                    )               
                )
                ...
                if (waveform_view and isinstance(input_dict[selected], str)):
                    ...
                    fig = px.line(data, markers=True)
                    fig.update_layout(showlegend=False)
                    details.append(dcc.Graph(id='wf-view',
                        config={'displayModeBar': False},
                        figure=fig
                        )
                    )
                    ...
                    details.append(dcc.Link(
                        "open this waveform",
                        href=("/config/waveforms/%s" 
                            % input_dict[selected]),
                        id="wf-link"
                    ))
                else:
                    if (isinstance(input_dict[selected], tuple)):
                        if (updated_value is not None and 
                            isinstance(updated_value, tuple)):
                            input_dict[selected] = updated_value
                            return "OK"
                        details.append(
                            dbc.InputGroup(
                                [dbc.InputGroupText("("),
                                dbc.Input(id=(f"tuple-0"),
                                placeholder="Enter value...",
                                type="text",
                                value=("%s" % input_dict[selected][0]),
                                ),
                                dbc.InputGroupText(","),
                                dbc.Input(id=(f"tuple-1"),
                                placeholder="Enter value...",
                                type="number",
                        ...
                        details.append(
                            dbc.InputGroup(
                                [
                                dbc.Input(id="input-field", placeholder="Enter value...",
                            type="text", value=("%s" % input_dict[selected])),
                                dbc.Button("Update", id="update-button", n_clicks=0),
                                ]
                            )                            
                        )
