...
@app.callback(
    [Output("component-gui", "children"),
     Output("component-doc", "children")
    ],
...
def component_editor(selected_component, inputs=None):
    ...
    if  hasattr(component_class, "add") and callable(component_class.add) and inputs[0] == None and inputs[1] == None:
        ...
        for index, name in enumerate(argument_names):
            ...
            possible_addons = [primitive_components.Operation,
            ...
            for addon in possible_addons:
                ...
                if (addon in typing.get_args(argument_types[name])):
                    ...
                    forget, gui2, add_index  = class_editor_and_gui(
                        ...
                        inputs,
                        add_index,
                        input_types=["optional-class-input", "optional-class-port-list"]       
                    )
                    number_of_input_fields = len(gui2) - 2
                    gui2.append(dbc.Input(
                    ...
                    gui2.append(dbc.Button("Add option to component",
                        color="secondary",
                        className="me-1",
                        id={
                        "index":add_index[0]+1,
        ...
        component_gui.append(dcc.Dropdown(
            options=[],
            value=[],
            multi=True,
            placeholder="add optional objects using the menu below",
...
def class_editor_and_gui(selected_class,
        setup,
        component_gui,
        python_command,
        inputs,
    ...
    for index_arg, name in enumerate(argument_names):
        ...
        if (argument_types[name] == str):
            ...
            if (inputs[0] is not None):
                ...
                python_command += (",\n  " if (input_index[1]>0 or input_index[0]>0)
                    else "") + f"{value}"
            else:
                component_gui.append(
                    dbc.InputGroup(
                        [dbc.InputGroupText(name),
                        dbc.Input(placeholder="float",
                            type="number",
                            id = {
                                "type":input_types[0],
                                "index":input_index[0],
            ...
            if (inputs[0] is not None):
                python_command += (",\n  " if (input_index[1]>0 or input_index[0]>0)
                    else "") + f"[{inputs[0][input_index[0]]}]"
            else:
                component_gui.append(
                    dbc.InputGroup(
                        [
                            dbc.InputGroupText(name),
                            dbc.Textarea(placeholder="float, float, ...",
                                id = {
                                    "type":input_types[0],
                                    "index":input_index[0],
                                }
            ...
            if (inputs[0] is not None):
                ...
                component_gui.append(
                    dbc.InputGroup(
                        [
                            dbc.InputGroupText(name),
                            dbc.Textarea(placeholder="(0 or 1, duration in ns), (... , ...), ...",
                                id = {
                                    "type":input_types[0],
                                    "index":input_index[0],
                                }
            ...
            if (inputs[0] is not None):
                ...
                python_command += (",\n" if (input_index[0]>0 or input_index[1]>0)
                    else "") + command
            else:
          ...
          [
            ...
            if (inputs[0] is not None):
                ...
                for index, c in enumerate(controllers):
                    controllerList.append({'label': c.name,
                        'value': '%s, %d' % (c.name, index)})
                ...
                component_gui.append(
                    dbc.InputGroup(
                        [
                            ...
                            dbc.InputGroupText("port_id"),
                            dbc.Input(placeholder="integer", type="number", step=1,
                                id={
                                    "type":"input-port",
                                    "index": input_index[1],
                                },
            ...
            input_index[1] += 1
        elif (argument_types[name] ==
          typing.List[primitive_components.Waveform]):
            ...
            if (inputs[0] is not None):
                ...
                for index, c in enumerate(waveforms):
                    waveformList.append({'label': c.name,
                        'value': index})
            ...
            if (inputs[0] is not None):
                ...
                for index, c in enumerate(elements):
                    elementList.append({'label': c.name,
                        'value': index})
            ...
            if (inputs[0] is not None):
                ...
                if selection == 1:
                    python_command += (",\n  " if (input_index[1]>0 or input_index[0]>0)
                        else "") + f"setup.find_instances(ConstantIntegrationWeights)[{index}]"
                else:
                    # selection == 2
                    python_command += (",\n  " if (input_index[1]>0 or input_index[0]>0)
                        else "") + f"setup.find_instances(ArbitraryIntegrationWeights)[{index}]"
            else:
...
def get_docstring(selected_class):
    if (selected_class.__init__ is not None and
        selected_class.__init__.__doc__ is not None):
