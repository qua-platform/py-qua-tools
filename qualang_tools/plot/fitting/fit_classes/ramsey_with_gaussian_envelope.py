"""
Should clean up this file to inherit from the Ramsey class
"""
...
class RamseyWithGaussianEnvelope(FittingBaseClass):
    ...
    def __init__(
        ...
        assert self.guess_T1 is not None, 'Must provide T1 for this function to prevent underconstrained optimisation.'

        if verbose:
            self.print_initial_guesses()

        self.fit_data(p0=[1, 1, self.guess_phase, 1, 1])
    ...
    def func(self, x_var, a0, a1, a2, a3, a4, a5):
        cos = np.cos(2 * np.pi * x_var * (a0 * self.guess_freq) + (a2 * self.guess_phase))
        decay1 = np.exp(-x_var / (2 * self.guess_T1))
        decay2 = np.exp(-x_var ** 2 / ((a1 * self.guess_T2) ** 2))

        return a3 * self.peaks[0] * cos * (decay1 * decay2) + (a4 * self.initial_offset) + (
                ((a5 * self.final_offset) - (a4 * self.initial_offset)) / ((x_var[-1] - x_var[0]))) * x_var
    ...
    def generate_out_dictionary(self):
        ...
        self.out = {
            ...
            "T1": [(self.guess_T1) * self.x_normal, self.guess_T1 * self.x_normal],
            "T2": [(self.guess_T2 * self.popt[1]) * self.x_normal, self.perr[1] * self.guess_T2 * self.x_normal],
            "amp": [self.peaks[0] * self.popt[3] * self.y_normal, self.perr[3] * self.peaks[0] * self.y_normal],
            "initial_offset": [
                self.popt[4] * self.initial_offset * self.y_normal,
                self.perr[4] * self.initial_offset * self.y_normal,
            ],
            "final_offset": [
                self.final_offset * self.popt[5] * self.y_normal,
                self.perr[5] * self.final_offset * self.y_normal,
            ],
        }
    ...
    def print_initial_guesses(self):
        print(
            ...
            f" T1 = {self.guess_T1 * self.x_normal:.3f}, \n"
