...
class TwoStateDiscriminator(StateDiscriminator):
    ...
    def _IQ_mu_sigma(self, b_vec):
        ...
        I_res = np.sum(out1 * (cos * np.real(b_vec) + sin * np.imag(-b_vec)), axis=1) \
                + np.sum(out2 * (cos * np.imag(b_vec) + sin * np.real(b_vec)) * sign, axis=1)
        Q_res = np.sum(out2 * (cos * np.real(b_vec) + sin * np.imag(-b_vec)), axis=1) \
                - np.sum(out1 * (cos * np.imag(b_vec) + sin * np.real(b_vec)) * sign, axis=1)
        I_res *= 2 ** -12
        Q_res *= 2 ** -12
        ...
        for i in range(self.num_of_states):
            ...
            gmm = mixture.GaussianMixture(n_components=1, covariance_type='spherical', tol=1e-12,
