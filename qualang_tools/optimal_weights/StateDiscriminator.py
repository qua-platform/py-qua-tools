...
class StateDiscriminator:
    ...
    def train(
        ...
        **execute_args,
        ...
        np.savez(self.path, weights=weights, bias=bias, smearing=self.smearing, meas_len=self.meas_len)
        self.saved_data = {"weights": weights, "bias": bias, "smearing": self.smearing, "meas_len": self.meas_len}
        self.finish_train = 1
        self._update_config()
