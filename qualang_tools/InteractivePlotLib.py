...
class InteractivePlotLib_Figure:
    def __init__(self, fig, doc, master_obj):
        ...
        self.cid2 = self.fig.canvas.mpl_connect(
            "key_press_event", self.keyboard_click)
    ...
    class LineSelected:
        ...
        def correct_order(self_line, change):
            ...
            to_replace_idx = np.where(
                np.array(sup_self.correct_order) == new_order)[0]
    ...
    class ConvertLogLin:
        def __init__(self_loglin, type_list):  # x,y
            ...
            for t in type_list:
                if t == "log":
                    self_loglin.convert.append(
                        lambda x: np.log10(np.maximum(x, 1e-16)))
                ...
                if command == ":lin" or command == ":linear":
                    ...
                    try:
                        ...
                        if type == "xlabel":
                            ...
                            for line in self_state.sup_self.ax.get_lines():
                                line.set_xdata(
                                    convert_function(line.get_xdata()))
                            self_state.sup_self.ax.set_xlim(
                                convert_function(
                            ...
                            for line in self_state.sup_self.ax.get_lines():
                                line.set_ydata(
                                    convert_function(line.get_ydata()))
                            self_state.sup_self.ax.set_ylim(
                                convert_function(
...
class Single_Fit:
    ...
    def fit_polygon(self, line, options):
        ...
        self.ddx0 = np.array(
            np.real([i for i in np.roots(self.ddp) if np.isreal(i)]))
