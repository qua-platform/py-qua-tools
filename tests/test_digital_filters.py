...
def test_single_calc_filter_taps(calc_correction):
    ...
    feedforward, feedback = calc_filter_taps(
        fir=None, exponential=low_pass, highpass=high_pass, bounce=None, delay=None, Ts=ts
    )
    print(f"\nfeedback: {feedback}")
    print(f"feedforward: {feedforward}")
    if low_pass is None:
...
def test_delay_calc_filter_taps(calc_correction):
    ...
    feedforward, feedback = calc_filter_taps(
        fir=None, exponential=low_pass, highpass=high_pass, bounce=None, delay=delay, Ts=ts
    )
    print(f"\nfeedback: {feedback}")
    print(f"feedforward: {feedforward}")
    ...
    assert (
        feedforward[int(delay / ts) :]
        == calc_filter_taps(fir=None, exponential=low_pass, highpass=high_pass, bounce=None, delay=0, Ts=ts)[0]
    )
    assert (
        feedback == calc_filter_taps(fir=None, exponential=low_pass, highpass=high_pass, bounce=None, delay=0, Ts=ts)[1]
    )
...
def test_fir_calc_filter_taps(calc_correction):
    ...
    feedforward, feedback = calc_filter_taps(
        fir=hann(20) / np.sum(hann(20)), exponential=low_pass, highpass=high_pass, bounce=None, delay=None, Ts=ts
    )
    print(f"\nfeedback: {feedback}")
    print(f"feedforward: {feedforward}")

    assert feedforward == list(
        np.convolve(
            calc_filter_taps(fir=None, exponential=low_pass, highpass=high_pass, bounce=None, delay=None, Ts=ts)[0],
