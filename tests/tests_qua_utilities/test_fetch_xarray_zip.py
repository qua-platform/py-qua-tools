...
def test_qua_zip_native(qmm):
    ...
    assert_dims_and_shape(
        xarray_data, "val_st", ("shot", "qb_amp", "frequency"), (10, len(native_zip_values), len(frequencies))
    )

    for i in range(10):
        assert (
            xarray_data["val_st"][i] == i
        ).all(), f"val_st[{i}]: expected all values to be {i}, got {xarray_data['val_st'][i].values}"
...
def test_qua_zip_qua(qmm):
    ...
    for i, f in enumerate(frequencies):
        assert np.allclose(
            xarray_data["freq_st"][i], f
        ), f"freq_st[{i}]: expected all values to be {f}, got {xarray_data['freq_st'][i].values}"
