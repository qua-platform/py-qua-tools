...
_TOO_OLD = f"{pp.MIN_QM_QUA_VERSION.major}.0.0"
...
def test_qua_iterables_supported_version_gate(monkeypatch):
    ...
    monkeypatch.setattr(pp, "_installed_qm_qua_version", lambda: str(pp.MIN_QM_QUA_VERSION))
    assert pp.qua_iterables_supported() is True

    # A clearly newer version is supported.
    newer = f"{pp.MIN_QM_QUA_VERSION.major + 1}.0.0"
